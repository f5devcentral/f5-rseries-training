...
F5 understands migrating configurations to new platforms can be a challenge and we’ve developed tooling that will help customers migrate existing BIG-IP configurations into rSeries tenants. Originally, F5 promoted the F5 BIG-IP Journeys application that assisted with migrating a UCS based configurations from older platforms such as iSeries to current TMOS versions running as tenants on the rSeries platforms.
...
In addition to the Journeys tool and the platform-migrate utility, there is a migration based Ansible collection that can be leveraged to setup both F5OS on a target rSeries system, and migrate a configuration into an F5OS based TMOS tenant:
