...
rSeries utilizes an opensource Kubernetes distribution called K3S. This is largely abstracted away from the administrator as they won’t be configuring or monitoring containers or Kubernetes components. In future releaases, some Kubernetes like features might start to be exposed, but it will likely be exposed through the F5OS CLI, GUI, or API’s. 
